...
| Migrations                 | drizzle-kit generate/migrate         | node-pg-migrate (removido)                  | drizzle-kit gera SQL a partir do schema TypeScript — fonte única de verdade; node-pg-migrate exigia JS de migração manual sem tipagem |
| Queries runtime            | `pg` SQL raw via `infra/database`    | drizzle-orm query builder                   | Repositórios já estabelecidos usam SQL explícito; drizzle-orm fica restrito a rodar migrations via `migrate()`                        |
